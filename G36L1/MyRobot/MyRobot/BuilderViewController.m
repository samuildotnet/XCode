...
- (IBAction)saveWorld {
    ...
    if ([NSKeyedArchiver archiveRootObject:self.drwView.paths toFile:[NSString stringWithFormat:@"%@/%@", [[NSBundle mainBundle] resourcePath], self.worldName]]) {
//        NSLog(@"save succses!");
    }
}
...
- (IBAction)addLine:(UITapGestureRecognizer *)sender {
    if (CGPointEqualToPoint(self.startPoint, CGPointZero)) {
        int x = [sender locationInView:self.drwView].x/HERO_SIZE;
        int y = [sender locationInView:self.drwView].y/HERO_SIZE;
        self.startPoint = CGPointMake(x*HERO_SIZE, y*HERO_SIZE);
